...
import com.farm.R;
import com.farm.adapter.ContractDepartment_Adapter;
...
{
    boolean isfinishCollection = false;
    ...
    Polyline polyline_boundary = null;
    List<LatLng> list_latlng_boundary = new ArrayList<>();
    ...
    ContractDepartment_Adapter contractDepartMent_Adapter;
    ...
    List<LatLng> listlatlng_parkboundary = new ArrayList<>();
    ...
    Button btn_addArea;
    @ViewById
    Button btn_CompleteCollection;
    @ViewById
    Button btn_saveCurrentLocation;
    @ViewById
...
//        cb_lock.setVisibility(View.VISIBLE);
        ...
        btn_addArea.setVisibility(View.GONE);
        btn_addmore.setVisibility(View.GONE);
        btn_showlayer.setVisibility(View.GONE);
        btn_setting.setVisibility(View.GONE);
        btn_CompleteCollection.setVisibility(View.VISIBLE);
        btn_saveCurrentLocation.setVisibility(View.VISIBLE);
    }
    ...
    void btn_saveCurrentLocation()
    {
        ...
        if (location_currentpoint != null)
        {
            ...
            tencentMap.removeOverlay(polyline_boundary);
            ...
            polyline_boundary.setColor(this.getResources().getColor(R.color.black));
            polyline_boundary.setWidth(4f);
            Overlays.add(polyline_boundary);
        } else
        {
            Toast.makeText(MakeLayer_Farm.this, "位置获取中...", Toast.LENGTH_LONG).show();
        }
    ...
    void btn_complete()
    {
        ...
        tencentMap.setOnMapClickListener(new TencentMap.OnMapClickListener()
        {
            ...
            public void onMapClick(LatLng latlng)
            {
                ...
                {
                    Toast.makeText(MakeLayer_Farm.this, "保存中...", Toast.LENGTH_LONG).show();
                    ...
                    List<CoordinatesBean> list_coordinate = new ArrayList<CoordinatesBean>();
                    for (int i = 0; i < listlatlng_parkboundary.size(); i++)
                    {
                        CoordinatesBean coordinatesBean = new CoordinatesBean();
                        coordinatesBean.setLat(String.valueOf(listlatlng_parkboundary.get(i).getLatitude()));
                        coordinatesBean.setLng(String.valueOf(listlatlng_parkboundary.get(i).getLongitude()));
                        coordinatesBean.setUuid(uuid_polygon);
                        coordinatesBean.setRegistime(utils.getTime());
                        list_coordinate.add(coordinatesBean);
                    }
                    StringBuilder builder = new StringBuilder();
                    builder.append("{\"PolygonBeanList\": [");
                    builder.append(JSON.toJSONString(polygonBean));
                    builder.append("], \"CoordinatesBeanLists\": ");
                    builder.append(JSON.toJSONString(list_coordinate));
                    builder.append("} ");
...
//                addMarker_Paint(0, latlng, R.drawable.location_start);
                ...
                line.setColor(MakeLayer_Farm.this.getResources().getColor(R.color.black));
                line.setWidth(4f);
                Overlays.add(line);
            }
        });
                ...
                public void onMapClick(LatLng latlng)
                {
                    ...
                    if (latlng_clickpostion != null)
                    {
                        for (int i = 0; i < list_polygon.size(); i++)
                        {
                            ...
                            {
                                Toast.makeText(MakeLayer_Farm.this, "在范围内", Toast.LENGTH_SHORT).show();
                            }
                        }
...
//        lp.alpha = 0.7f;
//        AddOrderMap.this.getWindow().setAttributes(lp);
        pv_command.findViewById(R.id.btn_standardprocommand).setOnClickListener(MakeLayer_Farm.this);
        pv_command.findViewById(R.id.btn_nonstandardprocommand).setOnClickListener(MakeLayer_Farm.this);
        pv_command.findViewById(R.id.btn_nonprocommand).setOnClickListener(MakeLayer_Farm.this);
    }
...
//        AddOrderMap.this.getWindow().setAttributes(lp);
        ...
        dl_zs_adapter = new DL_ZS_Adapter(MakeLayer_Farm.this, list_zs);
...
//                }
                ...
                SqliteDb.save(MakeLayer_Farm.this, zs);
                if (pv_command.isShown())
                {
                    list_zs = SqliteDb.getZS(MakeLayer_Farm.this, ZS.class, commembertab.getareaId());
...
//                SqliteDb.save(CZ_MakeMap_MakeLayer.this, coordinatesBean);
    ...
    public void showDialog_editArea(final PolygonBean polygonBean)
    {
        ...
        tv_belong.setText(polygonBean.getparkName() + polygonBean.getareaName() + polygonBean.getContractname());
        et_areanote.setText(polygonBean.getNote());
        et_numberofplant.setText(polygonBean.getNumofplant());
        tv_belong.setOnClickListener(new View.OnClickListener()
        {
            @Override
            public void onClick(View v)
            {
                tv_belong.setText("获取中...");
        ...
        btn_sure.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                if (tv_belong.getText().toString().equals(""))//没选择
                {
                    Toast.makeText(MakeLayer_Farm.this, "请选择属于这一项", Toast.LENGTH_LONG).show();
                } else if(departmentselected==null)
                {
                    customDialog_edtiArea.dismiss();
                    updateAreaInfo(polygonBean.getUuid(), et_areanote.getText().toString(), et_numberofplant.getText().toString(), polygonBean.getparkId(), polygonBean.getparkName(), polygonBean.getAreaId(), polygonBean.getareaName(), polygonBean.getContractid(), polygonBean.getContractname());
                }else
                {
                    customDialog_edtiArea.dismiss();
                    updateAreaInfo(polygonBean.getUuid(), et_areanote.getText().toString(), et_numberofplant.getText().toString(), departmentselected.getParkid(), departmentselected.getParkname(), departmentselected.getAreaid(), departmentselected.getAreaname(), departmentselected.getContractid(), departmentselected.getContractname());
                }
            }
        }); btn_cancle.setOnClickListener(new View.OnClickListener()
    {
        ...
        customDialog_edtiArea.setOnCancelListener(new DialogInterface.OnCancelListener()
        {
            @Override
            public void onCancel(DialogInterface dialog)
            {
...
//                SqliteDb.save(CZ_MakeMap_MakeLayer.this, coordinatesBean);
    ...
    public void showDialog_deletetip_custom(final PolygonBean polygonbean_selected, final String type)
    {
        ...
        tv_delete.setText(polygonbean_selected.getNote());
        btn_cancle = (Button) dialog_layout.findViewById(R.id.btn_cancle);
        ...
        btn_sure.setOnClickListener(new View.OnClickListener()
        {
            ...
            {
                customdialog_editdlinfor.dismiss();
                deletePolygon_custom(polygonbean_selected, type);
            }
        });
        btn_cancle.setOnClickListener(new View.OnClickListener()
        {
    ...
    public void showDialog_deletetip(final PolygonBean polygonbean_selected, final String type)
    {
        ...
        {
            tv_delete.setText("即将删除:" + polygonbean_selected.getNote() + "\n" + "此操作会将该承包区所属的片区下的所有承包区都删除，确定要删除吗？");
        } else
        {
            tv_delete.setText("确定删除吗？");
        }
        btn_sure.setOnClickListener(new View.OnClickListener()
        {
            @Override
            ...
            {
                customdialog_editdlinfor.dismiss();
                deletePolygon(polygonbean_selected, type);
            }
        });
        btn_cancle.setOnClickListener(new View.OnClickListener()
        {
    ...
    public void showDialog_CompleteCollection()
    {
        ...
        btn_sure.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                isfinishCollection = true;
                btn_CompleteCollection.setVisibility(View.GONE);
                customdialog_editdlinfor.dismiss();
                tv_tip.setVisibility(View.VISIBLE);
                tv_tip.setBackgroundResource(R.color.bg_job);
                ...
                tencentMap.setOnMapClickListener(new TencentMap.OnMapClickListener()
                {
                    ...
                    public void onMapClick(LatLng latLng)
                    {
                        ...
                        {
                            showDialog_SaveCollectionInfo(latLng);
                        } else
                        {
    ...
    public void showDialog_SaveCollectionInfo(final LatLng latlng)
    {
        ...
        btn_sure.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                polygonBean.setOrders("");
                polygonBean.setXxzt("0");
                polygonBean.setNote(et_areanote.getText().toString());
        ...
        try
        {
            ...
            if (list_parktab != null)
            {
                for (int i = 0; i < list_parktab.size(); i++)
                {
                    ...
                    boolean isexistpark = SqliteDb.isexistpark(MakeLayer_Farm.this, list_parktab.get(i).getid());
                    if (isexistpark)
                    {
                        isdrawer_park = "1";
                    }
                    ...
                    if (list_areatab != null)
                    {
                        for (int j = 0; j < list_areatab.size(); j++)
                        {
                            ...
                            boolean isexistarea = SqliteDb.isexistarea(MakeLayer_Farm.this, list_parktab.get(i).getid(), list_areatab.get(j).getid());
                            if (isexistarea)
                            {
                                isdrawer_area = "1";
                            }
                            ...
                            if (list_contractTab != null)
                            {
                                for (int k = 0; k < list_contractTab.size(); k++)
                                {
                                    ...
                                    boolean isexistcontract = SqliteDb.isexistcontract(MakeLayer_Farm.this, list_parktab.get(i).getid(), list_areatab.get(j).getid(), list_contractTab.get(k).getid());
                                    if (isexistcontract)
                                    {
                                        isdrawer_contract = "1";
                                    }
    ...
    public void showDialog_SelectOperation()
    {
        ...
        JSONArray jsonArray = FileHelper.getAssetsJsonArray(MakeLayer_Farm.this, "operation");
        for (int i = 0; i < jsonArray.size(); i++)
        {
            list_operation.add(jsonArray.get(i).toString());
        }
        operation_adapter = new Operation_Adapter(MakeLayer_Farm.this, list_operation);
                ...
                } else if (operation.equals("道路"))
                {
                    ...
                    tencentMap.setOnMapClickListener(new TencentMap.OnMapClickListener()
                    {
                        ...
                        public void onMapClick(LatLng latlng)
                        {
                            ...
                            line.setColor(MakeLayer_Farm.this.getResources().getColor(R.color.red));
                            line.setWidth(8f);
                            Overlays.add(line);
                        }
                    });
...
//                showNeedPlanBoundary(list_coordinatesbean);
                ...
                {
                    Toast.makeText(MakeLayer_Farm.this, "这个区域已画！请选择其他区域", Toast.LENGTH_SHORT).show();
                    return;
                }
...
//                    });
                } else
                {
                    ...
                    getNeedPlanMap(departmentselected.getType(), departmentselected.getParkid(), departmentselected.getAreaid(), departmentselected.getContractid());
        ...
        tencentMap.setOnMapClickListener(new TencentMap.OnMapClickListener()
        {
            ...
            public void onMapClick(LatLng latLng)
            {
                ...
                if (lastselect_latlng != null)
                {
                    ...
                    {
                        Toast.makeText(MakeLayer_Farm.this, "错误操作！请不要一次切割多条边界！", Toast.LENGTH_SHORT).show();
                        //撤销该次操作，并重置参数
...
//                SqliteDb.save(CZ_MakeMap_MakeLayer.this, coordinatesBean);
            ...
            polygonBean_notselect = new PolygonBean();
            ...
            polygonBean_notselect.setAreaId("-1");//画选片区，list_notselect则只能为园区的未规划图，所以为空
            polygonBean_notselect.setareaName("");//画选片区，list_notselect则只能为园区的未规划图，所以为空
            polygonBean_notselect.setContractid("-1");
...
//                SqliteDb.save(CZ_MakeMap_MakeLayer.this, coordinatesBean);
            ...
            polygonBean_notselect.setContractid("-1");//画选承包区，list_notselect则只能为片区的未规划图，所以为空
            polygonBean_notselect.setContractname("");
            polygonBean_notselect.setBatchid("");
            polygonBean_notselect.setCoordinatestime(utils.getTime());
            polygonBean_notselect.setRegistime(utils.getTime());
...
//                SqliteDb.save(CZ_MakeMap_MakeLayer.this, coordinatesBean);
        ...
        PolygonBeanList.add(polygonBean);
        PolygonBeanList.add(polygonBean_notselect);
        CoordinatesBeanLists.addAll(list_coordinate_select);
        CoordinatesBeanLists.addAll(list_coordinate_notselect);
        StringBuilder builder = new StringBuilder();
        builder.append("{\"PolygonBeanList\": ");
        builder.append(JSON.toJSONString(PolygonBeanList));
        builder.append(", \"CoordinatesBeanLists\": ");
...
//                SqliteDb.save(CZ_MakeMap_MakeLayer.this, coordinatesBean);
        ...
        builder.append("{\"PolygonBeanList\": [");
        builder.append(JSON.toJSONString(polygonBean));
//        builder.append(JSON.toJSONString(polygonBean_notselect));
        builder.append("], \"CoordinatesBeanLists\": ");
        builder.append(JSON.toJSONString(list_coordinate_select));
//        builder.append(JSON.toJSONString(list_coordinate_notselect));
    ...
    public void saveCoordinatesBean(String type, String isInnerPoint, String isCenterPoint, String orders, LatLng latlng, DepartmentBean departmentbean)
    {
        ...
        coordinatesBean.setUuid(uuid);
        coordinatesBean.setRegistime(utils.getTime());
        SqliteDb.save(MakeLayer_Farm.this, coordinatesBean);
    }
    ...
    public void showDialog_OperatePolygon_Custom(final PolygonBean polygonbean_selected, final String type)
    {
        ...
        btn_delete.setOnClickListener(new View.OnClickListener()
        {
            ...
            {
                customdialog_operatepolygon.dismiss();
                showDialog_deletetip_custom(polygonbean_selected, type);
            }
        });
...
//                PolygonBean polygonbean = SqliteDb.getLayerbyuuid(CZ_MakeMap_MakeLayer.this, uuid);
        ...
        btn_change.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                showDialog_editpointinfo(polygonbean_selected);
                customdialog_operatepolygon.dismiss();
            }
        });
        btn_cancle.setOnClickListener(new View.OnClickListener()
    ...
    public void showDialog_OperatePolygon_BoundaryPoint(final CoordinatesBean coordinatesBean, final String type, final String note)
    {
        ...
        btn_delete.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                list_coordinatesbean_boundary.remove(pos);//坐标点
                list_latlng_boundary.remove(pos);//坐标点
        ...
        btn_see.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                showDialog_polygonnote("经度" + coordinatesBean.getLat() + "\n纬度" + coordinatesBean.getLng());
                customdialog_operatepolygon.dismiss();
            }
        });
        btn_change.setOnClickListener(new View.OnClickListener()
...
//    }
    public void showDialog_OperateArea(final PolygonBean polygonbean_selected, final String type)
    {
        ...
        btn_see.setOnClickListener(new View.OnClickListener()
        {
            ...
            {
//                PolygonBean polygonbean = SqliteDb.getLayerbyuuid(CZ_MakeMap_MakeLayer.this, uuid);
                showDialog_polygonnote("名称:" + polygonbean_selected.getparkName() + polygonbean_selected.getareaName() + polygonbean_selected.getContractname() + "\n说明:" + polygonbean_selected.getNote());
                customdialog_operatepolygon.dismiss();
                reloadMap();
            }
        });
        ...
        customdialog_operatepolygon.setOnCancelListener(new DialogInterface.OnCancelListener()
        {
            @Override
            public void onCancel(DialogInterface dialog)
            {
    ...
    {
        commembertab = AppContext.getUserInfo(MakeLayer_Farm.this);
        getActionBar().hide();
        TencentLocationRequest request = TencentLocationRequest.create();
        TencentLocationManager locationManager = TencentLocationManager.getInstance(MakeLayer_Farm.this);
...
//        tencentMap.addTileOverlay(tileOverlayOptions);
    ...
    void afterOncreate()
    {
        ...
        map = new HashMap<>();
        if (!utils.isOPen(MakeLayer_Farm.this))
        {
            utils.openGPSSettings(MakeLayer_Farm.this);
        }
        initParam();
        initMapData();
        initMarkOnclick();
...
//    }
    ...
    public void onLocationChanged(TencentLocation location, int error, String reason)
    {
        if (TencentLocation.ERROR_OK == error) // 定位成功
        {
            ...
            //全局记录坐标
//            AppContext appContext = (AppContext) MakeLayer_Farm.this.getApplication();
...
//            appContext.setLOCATION_Y(String.valueOf(location_latLng.getLongitude()));
            ...
            {
                if (!isfinishCollection && !cb_lock.isSelected())
                {
                    tencentMap.animateTo(location_latLng);
                    location_currentpoint = location_latLng;
                    tencentMap.removeOverlay(Marker_currentlocation);
//                    Marker_currentlocation = addMarker(location_latLng, R.drawable.location_start);
                    Marker_currentlocation = addLocationMarker("location_point", R.drawable.location1, getResources().getColor(R.color.red), location_latLng);
...
//                Overlays.add(line);
                ...
                parkpolygon = drawPolygon(0f, listlatlng_park, R.color.bg_yellow, 2, R.color.bg_text);
                Overlays.add(parkpolygon);
            } else if (drawerType.equals("园区") && !isStopPaint)
            {
                if (isfirstpoint)
                {
                    isfirstpoint = false;
                ...
                Toast.makeText(MakeLayer_Farm.this, "paintting...", Toast.LENGTH_SHORT).show();
                tencentMap.animateTo(location_latLng);

                listlatlng_parkboundary.add(location_latLng);
                Overlays.remove(parkpolygon);
                parkpolygon = drawPolygon(0f, listlatlng_parkboundary, Color.argb(150, 144, 144, 144), 2, R.color.transparent);
                parkpolygon.setVisible(false);
                Overlays.add(parkpolygon);

                PolylineOptions lineOpt = new PolylineOptions();
                lineOpt.add(lastlatLng_paintpark);
                lastlatLng_paintpark = location_latLng;
                lineOpt.add(location_latLng);
                Polyline line = tencentMap.addPolyline(lineOpt);
//                line.setGeodesic(true);
...
//        marker.set2Top();
        View view = View.inflate(MakeLayer_Farm.this, R.layout.addfirstview, null);
    ...
    private Marker addSecondView(LatLng latLng, String uuid, String note)
    {
        ...
        list_centermark.add(marker);
//        marker.set2Top();
        View view = View.inflate(MakeLayer_Farm.this, R.layout.addsecondview, null);
    ...
    private Marker addThirdView(LatLng latLng, String uuid, String note)
    {
        ...
        list_centermark.add(marker);
//        marker.set2Top();
        View view = View.inflate(MakeLayer_Farm.this, R.layout.addthirdview, null);
    ...
    private Marker addLocationMarker(String type, int icon, int textcolor, LatLng latLng)
    {
        Marker marker = tencentMap.addMarker(new MarkerOptions().position(latLng));
        View view = LayoutInflater.from(MakeLayer_Farm.this).inflate(R.layout.marker_sale, null);
        View view_marker = (View) view.findViewById(R.id.view_marker);
    ...
    private Marker addCustomMarkerWithProgressbar(int textcolor, LatLng latLng, String uuid, String note)
    {
        ...
        list_centermark.add(marker);
        View view = View.inflate(MakeLayer_Farm.this, R.layout.markerwithprogressbar, null);
        TextView textView = (TextView) view.findViewById(R.id.tv_note);
        LinearLayout ll_second = (LinearLayout) view.findViewById(R.id.ll_second);
        LinearLayout ll_first = (LinearLayout) view.findViewById(R.id.ll_first);
        LinearLayout ll_third = (LinearLayout) view.findViewById(R.id.ll_third);
    ...
    private Marker addCustomMarkerWithHouse(int textcolor, LatLng latLng, String uuid, String note)
    {
        ...
        list_centermark.add(marker);
//        marker.set2Top();
        View view = LayoutInflater.from(MakeLayer_Farm.this).inflate(R.layout.markerwithhouse, null);
        TextView textView = (TextView) view.findViewById(R.id.tv_note);
        if (note == null || note.equals(""))
        {
            textView.setText("暂无说明");
...
//        tencentMap.setZoom(18);// 设置地图显示大小
...
//        initMapOnclick();
    ...
    private void showThirdMarker()
    {
        ...
        {
            List<parktab> list_parktab = SqliteDb.getparktab(MakeLayer_Farm.this, commembertab.getuId());
            for (int i = 0; i < list_parktab.size(); i++)//每个园区
            {
                List<areatab> list_areatab = SqliteDb.getareatab(MakeLayer_Farm.this, list_parktab.get(i).getid());
                for (int k = 0; k < list_areatab.size(); k++)//每个片区
                {
                    List<contractTab> list_contractTab = SqliteDb.getcontracttab(MakeLayer_Farm.this, list_areatab.get(k).getid());
                    for (int m = 0; m < list_contractTab.size(); m++)//每个承包区
                    {
                        PolygonBean polygonBean_contract = SqliteDb.getLayer_contract(MakeLayer_Farm.this, list_parktab.get(i).getid(), list_areatab.get(k).getid(), list_contractTab.get(m).getid());
    ...
    private void showSecondMarker()
    {
        ...
        } else
        {
            ...
            for (int i = 0; i < list_parktab.size(); i++)//每个园区
            {
                ...
                for (int k = 0; k < list_areatab.size(); k++)//每个片区
                {
                    PolygonBean polygonBean_area = SqliteDb.getLayer_area(MakeLayer_Farm.this, list_parktab.get(i).getid(), list_areatab.get(k).getid());
    ...
    private void showFirstMarker()
    {
        ...
        } else
        {
            ...
            for (int i = 0; i < list_parktab.size(); i++)//每个园区
            {
                PolygonBean polygonBean_park = SqliteDb.getLayer_park(MakeLayer_Farm.this, list_parktab.get(i).getid());
        ...
        tencentMap.setOnMarkerClickListener(new TencentMap.OnMarkerClickListener()
        {
            ...
            public boolean onMarkerClick(Marker marker)
            {
                ...
                if (type.equals("normal"))
                {
                    ...
                    showDialog_OperatePolygon_Custom(polygonbean, type);
                }
                if (type.equals("boundary_point"))
                {
                    CoordinatesBean coordinatesBean = (CoordinatesBean) obj;
...
//        }
    ...
    private void updateAreaInfo(String uuid, String note, String numofplant, String parkId, String parkName, String areaId, String areaName, String contractid, String contractname)
    {
        RequestParams params = new RequestParams();
...
//        Bitmap bitmap = utils.drawable2Bitmap(drawable);
        marker = tencentMap.addMarker(new MarkerOptions().position(latlng));
//        marker.set2Top();
        TextView textView = new TextView(MakeLayer_Farm.this);
        ...
        textView.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                Toast.makeText(MakeLayer_Farm.this, "good", Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//                        showDialog_OperatePolygon();
    ...
    private void deletePolygon(final PolygonBean polygonbean_selected, String type)
    {
        commembertab commembertab = AppContext.getUserInfo(MakeLayer_Farm.this);
        RequestParams params = new RequestParams();
        params.addQueryStringParameter("uuid", polygonbean_selected.getUuid());
        params.addQueryStringParameter("type", type);
        params.addQueryStringParameter("action", "deletePolygon");
        HttpUtils http = new HttpUtils();
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            @Override
    ...
    private void deletePolygon_custom(final PolygonBean polygonbean_selected, String type)
    {
        commembertab commembertab = AppContext.getUserInfo(MakeLayer_Farm.this);
        RequestParams params = new RequestParams();
        params.addQueryStringParameter("uuid", polygonbean_selected.getUuid());
        params.addQueryStringParameter("action", "deletePolygon_custom");
        HttpUtils http = new HttpUtils();
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            @Override
    ...
    private void editPolygon(final PolygonBean polygonbean_selected)
    {
        commembertab commembertab = AppContext.getUserInfo(MakeLayer_Farm.this);
        RequestParams params = new RequestParams();
        params.addQueryStringParameter("uuid", polygonbean_selected.getUuid());
        params.addQueryStringParameter("note", polygonbean_selected.getNote());
        params.addQueryStringParameter("action", "editPolygon");
        HttpUtils http = new HttpUtils();
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            @Override
...
//                            marker.setVisible(false);
    ...
    private void getNeedPlanMap(final String witchArea, String parkId, String areaid, String contractid)
    {
        commembertab commembertab = AppContext.getUserInfo(MakeLayer_Farm.this);
        RequestParams params = new RequestParams();
        params.addQueryStringParameter("parkId", parkId);
        params.addQueryStringParameter("areaid", areaid);
        params.addQueryStringParameter("type", witchArea);
        ...
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo)
            {
                ...
                if (result.getResultCode() == 1)// -1出错；0结果集数量为0；结果列表
                {
                    ...
                    {
                        listNewData = JSON.parseArray(result.getRows().toJSONString(), PolygonBean.class);
                        polygonBean_needPlan = listNewData.get(0);
                        LatLng latlng = new LatLng(Double.valueOf(polygonBean_needPlan.getLat()), Double.valueOf(polygonBean_needPlan.getLng()));
//                        addCustomMarker(getResources().getColor(R.color.bg_blue), latlng, polygonBean_needPlan.getUuid(), polygonBean_needPlan.getNote());
                        Marker marker = addCustomMarker(polygonBean_needPlan, "normal", R.drawable.ic_flag_contract, getResources().getColor(R.color.white), latlng, polygonBean_needPlan.getUuid(), polygonBean_needPlan.getNote());
                        list_coordinatesbean = polygonBean_needPlan.getCoordinatesBeanList();
                        if (list_coordinatesbean != null && list_coordinatesbean.size() != 0)
                        {
                            showNeedPlanBoundary(list_coordinatesbean);
                        }
                    ...
                    {
                        Toast.makeText(MakeLayer_Farm.this, "给图层的底层还没制作，请先制作该图层的底层！", Toast.LENGTH_SHORT).show();
                    }
        ...
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo)
            {
                ...
                if (result.getResultCode() == 1)// -1出错；0结果集数量为0；结果列表
                {
                    if (result.getAffectedRows() != 0)
                    {
                        if (drawerType.equals("园区"))
                        {
                            lastlatLng_paintpark = null;
                            tv_tip.setVisibility(View.VISIBLE);
                            tv_tip.setText("你已成功画选一个区域！");
                            listlatlng_park = new ArrayList<>();
//                            reloadMap();
                        } else if (drawerType.equals("片区承包区"))
                        {
                            resetParamsAfterPaint();
//                            reloadMap();
                        } else if (drawerType.equals("整个区域"))
                        {
                            resetParamsAfterPaint();
//                            reloadMap();
                        } else if (drawerType.equals("画点"))
...
//                            reloadMap();
                        ...
                        } else if (drawerType.equals("定点采点"))
                        {
                            ...
                            btn_setting.setVisibility(View.VISIBLE);
                            btn_CompleteCollection.setVisibility(View.GONE);
                            btn_saveCurrentLocation.setVisibility(View.GONE);
                        }
                        pb_upload.setVisibility(View.GONE);
                        drawerType = "";
                        Toast.makeText(MakeLayer_Farm.this, "保存成功", Toast.LENGTH_SHORT).show();
                        reloadMap();
                    }
                ...
                } else
                {
                    pb_upload.setVisibility(View.GONE);
                    AppContext.makeToast(MakeLayer_Farm.this, "error_connectDataBase");
//                    Toast.makeText(CZ_MakeMap_MakeLayer.this, "保存失败，请重试！", Toast.LENGTH_SHORT).show();
                    return;
                }
