...
import com.farm.R;
import com.farm.adapter.Common_TodayPlantAdapter;
...
@EActivity(R.layout.ncz_pq_todayplant)
public class NCZ_PQ_TodayPlant extends Activity
{
	TimeThread timethread;
	SelectorFragment selectorUi;
	Fragment mContent = new Fragment();
	private Common_TodayPlantAdapter listAdapter;
	...
	void btn_add()
	{
		Intent intent = new Intent(NCZ_PQ_TodayPlant.this, AddPlant_.class);
		intent.putExtra("areaid", areaid);
		NCZ_PQ_TodayPlant.this.startActivity(intent);
	}
	...
	void afterOncreate()
	{
		commembertab commembertab = AppContext.getUserInfo(NCZ_PQ_TodayPlant.this);
		if (commembertab.getnlevel().toString().equals("0"))
		{
			btn_add.setVisibility(View.GONE);
		}
		dictionary = DictionaryHelper.getDictionaryFromAssess(NCZ_PQ_TodayPlant.this, "PG_MQ");
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		getActionBar().hide();
		appContext = (AppContext) NCZ_PQ_TodayPlant.this.getApplication();
	/*	IntentFilter intentfilter_update = new IntentFilter(AppContext.BROADCAST_UPDATEPLANT);
		NCZ_PQ_TodayPlant.this.registerReceiver(receiver_update, intentfilter_update);*/
...
/*	BroadcastReceiver receiver_update = new BroadcastReceiver()// 从扩展页面返回信息
	...
	private void getListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		...
		http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				{
					AppContext.makeToast(NCZ_PQ_TodayPlant.this, "error_connectDataBase");
					return;
				}
				// 数据处理
				...
				case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
					...
					if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
					{
						...
						if (newdata > 0)
						{
							NewDataToast.makeText(NCZ_PQ_TodayPlant.this, getString(R.string.new_data_toast_message, newdata), appContext.isAppSound(), R.raw.newdatatoast).show();
						} else
						{
							// NewDataToast.makeText(NCZ_PQ_TodayPlant.this,
				...
				} else if (size == -1)
				{
					...
					AppContext.makeToast(NCZ_PQ_TodayPlant.this, "load_error");
				}
			...
			{
				AppContext.makeToast(NCZ_PQ_TodayPlant.this, "error_connectServer");
			}
		});
	}
	...
	{
		listAdapter = new Common_TodayPlantAdapter(NCZ_PQ_TodayPlant.this, listData);
		list_footer = NCZ_PQ_TodayPlant.this.getLayoutInflater().inflate(R.layout.listview_footer, null);
		...
		frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				if (planttab == null)
					return;
				commembertab commembertab = AppContext.getUserInfo(NCZ_PQ_TodayPlant.this);
				AppContext.updateStatus(NCZ_PQ_TodayPlant.this, "0", planttab.getId(), "3", commembertab.getId());
				Intent intent = new Intent(NCZ_PQ_TodayPlant.this, ShowPlant_.class);
