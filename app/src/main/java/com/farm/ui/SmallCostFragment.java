 ...
 */
@EActivity(R.layout.fragment_small_cost)
public class SmallCostFragment extends Activity
{
    ...
    public void switchContent(Fragment from, Fragment to)
    {
        if (mContent != to)
        {
            ...
            if (!to.isAdded())
            { // 先判断是否被add过
                transaction.hide(from).add(R.id.container_small, to).commit(); // 隐藏当前的fragment，add下一个到Activity中
            } else
            {
