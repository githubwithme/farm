            ...
            public void onSuccess(ResponseInfo<String> responseInfo)
            {
                ...
                if (result.getResultCode() == 1)// -1出错；0结果集数量为0；结果列表
                {
                    if (result.getAffectedRows() != 0)
                    {
                        ...
                        if (!et_goodsname.getText().toString().equals(""))
                        {
                            Iterator<Wz_Storehouse> it = listNewData.iterator();
                            while (it.hasNext())
                            {
                                String value = it.next().getGoodsName();
                                if (value.indexOf(et_goodsname.getText().toString())==-1)
                                {
                                    it.remove();
                                }
                            }
