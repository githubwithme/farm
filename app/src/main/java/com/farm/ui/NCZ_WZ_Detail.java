        ...
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo)
            {
                ...
                if (result.getResultCode() == 1)// -1出错；0结果集数量为0；结果列表
                {
                    ...
                    goodsunit.setText(wz_detail.getCostUnit());


                    if (!wz_detail.getThree().equals(""))
                    {
                        goodsSpec.setText(wz_detail.getGoodsStatistical() + wz_detail.getGoodsunit()+ "/"  + wz_detail.getThree());
                        goodssum.setText(wz_detail.getFirs()+"/"+wz_detail.getSecNum()+wz_detail.getSec()+"\n"+wz_detail.getSec()+"/"+wz_detail.getThreeNum()+wz_detail.getThree());
                    } else if (wz_detail.getThree().equals("")&&!wz_detail.getSec().equals(""))
                    {
                        goodsSpec.setText(wz_detail.getGoodsStatistical() + wz_detail.getGoodsunit()+ "/"  + wz_detail.getSec());
                        goodssum.setText(wz_detail.getFirs()+"/"+wz_detail.getSecNum()+wz_detail.getSec());
                    }else
                    {
                        goodsSpec.setText(wz_detail.getGoodsStatistical() + wz_detail.getGoodsunit()  + "/"+ wz_detail.getFirs());
                        goodssum.setText(wz_detail.getFirs());
                    }
