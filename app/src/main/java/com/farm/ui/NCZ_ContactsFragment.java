...
import com.farm.R;
import com.farm.adapter.Adapter_UserList;
...
@EFragment
public class NCZ_ContactsFragment extends Fragment
{
    private Adapter_UserList listAdapter;
    private int listSumData;
    private List<commembertab> listData = new ArrayList<commembertab>();
    ...
    {
        View rootView = inflater.inflate(R.layout.ncz_contactsfragment, container, false);
        return rootView;
    }
        ...
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo)
            {
                ...
                if (result.getResultCode() == 1)// -1出错；0结果集数量为0；结果列表
                {
                    ...
                    {
                        listNewData = new ArrayList<commembertab>();
                        ...
                        switch (objtype)
                        {
                            case UIHelper.LISTVIEW_DATATYPE_NEWS:
                                ...
                                if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
                                {
                                    ...
                                    {
                                        for (commembertab commembertab1 : listNewData)
                                        {
                                            boolean b = false;
                                            for (commembertab commembertab2 : listData)
                                            {
                                                if (commembertab1.getId().equals(commembertab2.getId()))
                                                {
                                                    b = true;
                                                    break;
                                                }
                            ...
                            case UIHelper.LISTVIEW_DATATYPE_NEWS:
                                ...
                                if (listNewData.size() > 0)
                                {
                                    for (commembertab commembertab1 : listNewData)
                                    {
                                        ...
                                        if (!b) listData.add(commembertab1);
                                    }
                                } else
                                {
                                    listData.addAll(listNewData);
    ...
    {
        listAdapter = new Adapter_UserList(getActivity(), listData);
