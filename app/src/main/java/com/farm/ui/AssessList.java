...
import com.farm.R;
import com.farm.adapter.ListViewAssessAdapter;
...
@SuppressLint("NewApi")
@EActivity(R.layout.assesslist)
public class AssessList extends Activity implements OnClickListener
{
	private ListViewAssessAdapter listAdapter;
	...
	void ll_tab()
	{
		iv_dowm_tab.setBackground(getResources().getDrawable(R.drawable.upward));
		showPop_title();
	...
	private void getListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		...
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.testurl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				{
					AppContext.makeToast(AssessList.this, "error_connectDataBase");
					return;
				}
				...
				case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
					...
					if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
					{
						...
						if (newdata > 0)
						{
							NewDataToast.makeText(AssessList.this, getString(R.string.new_data_toast_message, newdata), appContext.isAppSound(), R.raw.newdatatoast).show();
						} else
						{
//							NewDataToast.makeText(AssessList.this, getString(R.string.new_data_toast_none), false, R.raw.newdatatoast).show();
						}
					}
					break;
				...
				} else if (size == -1)
				{
					...
					AppContext.makeToast(AssessList.this, "load_error");
				}
			...
			{
				AppContext.makeToast(AssessList.this, "error_connectServer");
			}
		});
	}
	...
	{
		listAdapter = new ListViewAssessAdapter(this, listData);
		...
		frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				if (plantgrowthtab == null)
					return;
				Intent intent = new Intent(AssessList.this, EveryDayAssessList_.class);
				intent.putExtra("plant", plantgrowthtab);// 因为list中添加了头部,因此要去掉一个
	...
	public void showPop_title()
	{
		...
		pv_tab.setOnKeyListener(new OnKeyListener()
		{
			...
			public boolean onKey(View v, int keyCode, KeyEvent event)
			{
				...
				{
					pw_tab.dismiss();
					iv_dowm_tab.setBackground(getResources().getDrawable(R.drawable.downward));
					return true;
				}
				return false;
			}
		...
		listview.setAdapter(new TitleAdapter(AssessList.this, data));
		listview.setOnItemClickListener(new OnItemClickListener()
		{
			...
			public void onItemClick(AdapterView<?> arg0, View v, int postion, long arg3)
			{
				...
				tv_title.setTextColor(getResources().getColor(R.color.light_blue));
				iv_dowm_tab.setBackground(getResources().getDrawable(R.drawable.downward));
				pw_tab.dismiss();
			}
		});
