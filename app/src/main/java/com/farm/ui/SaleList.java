...
import com.farm.R;
import com.farm.adapter.ListViewSaleAdapter;
...
@SuppressLint("NewApi")
@EActivity(R.layout.salelist)
public class SaleList extends Activity implements OnClickListener
{
	SelectorFragment selectorUi;
	Fragment mContent = new Fragment();
	private ListViewSaleAdapter listAdapter;
	...
	void btn_good()
	{
		Intent intent = new Intent(SaleList.this, GoodList_.class);
	...
	void btn_product()
	{
		Intent intent = new Intent(SaleList.this, ProductBatchList_.class);
		startActivity(intent);
	}
	...
	{
		// showPop_addcommand();
		Intent intent = new Intent(SaleList.this, AddSpecialCost_.class);
		startActivity(intent);
	}
	...
	void afterOncreate()
	{
		dictionary = DictionaryHelper.getDictionaryFromAssess(SaleList.this, "XS");
...
/*	BroadcastReceiver receiver_update = new BroadcastReceiver()// 从扩展页面返回信息
	...
	{
		commembertab commembertab = AppContext.getUserInfo(SaleList.this);
		...
		http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				{
					AppContext.makeToast(SaleList.this, "error_connectDataBase");
					return;
				}
				// 数据处理
				...
				case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
					...
					if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
					{
						...
						if (newdata > 0)
						{
							NewDataToast.makeText(SaleList.this, getString(R.string.new_data_toast_message, newdata), appContext.isAppSound(), R.raw.newdatatoast).show();
						} else
						{
							// NewDataToast.makeText(SaleList.this,
				...
				} else if (size == -1)
				{
					...
					AppContext.makeToast(SaleList.this, "load_error");
				}
			...
			{
				AppContext.makeToast(SaleList.this, "error_connectServer");
			}
		});
	}
	...
	{
		listAdapter = new ListViewSaleAdapter(SaleList.this, listData);
		list_footer = SaleList.this.getLayoutInflater().inflate(R.layout.listview_footer, null);
		...
		frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				{
					Intent intent = new Intent(SaleList.this, MainSaleDetail_.class);
					intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
					startActivity(intent);
				} else if (planttab.getplantType().equals("1"))
				{
					Intent intent = new Intent(SaleList.this, SmallSaleDetail_.class);
					intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
					startActivity(intent);
				} else if (planttab.getplantType().equals("11"))
				{
					Intent intent = new Intent(SaleList.this, LargeSaleDetail_.class);
					intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
					startActivity(intent);
				}
			}
	...
	public void showPop_title()
	{
		...
		listview.setAdapter(new TitleAdapter(SaleList.this, data));
	...
	public void onClick(View v)
	{
		...
		case R.id.btn_standardprocommand:
			intent = new Intent(SaleList.this, AddStandardCommand_.class);
			startActivity(intent);
			break;
		case R.id.btn_nonstandardprocommand:
			intent = new Intent(SaleList.this, AddNotStandardCommand_.class);
			startActivity(intent);
			break;
		case R.id.btn_nonprocommand:
			intent = new Intent(SaleList.this, AddNotProductCommand_.class);
			startActivity(intent);
			break;
