...
import com.farm.R;
import com.farm.adapter.ListViewProductBatchAdapter;
...
@SuppressLint("NewApi")
@EActivity(R.layout.productbatchlist)
public class ProductBatchList extends Activity implements OnClickListener
{
	SelectorFragment selectorUi;
	Fragment mContent = new Fragment();
	private ListViewProductBatchAdapter listAdapter;
	...
	void btn_good()
	{
		Intent intent = new Intent(ProductBatchList.this, GoodList_.class);
	...
	void btn_product()
	{
		Intent intent = new Intent(ProductBatchList.this, SaleList_.class);
		startActivity(intent);
	}
	...
	{
		// dictionary =
		// DictionaryHelper.getDictionaryFromAssess(ProductBatchList.this,
		// "CP");
	...
	public void switchContent(Fragment from, Fragment to)
	{
		if (mContent != to)
		{
			...
			if (!to.isAdded())
			{ // 先判断是否被add过
				transaction.hide(from).add(R.id.top_container_product, to).commit(); // 隐藏当前的fragment，add下一个到Activity中
			} else
			{
	...
	{
		commembertab commembertab = AppContext.getUserInfo(ProductBatchList.this);
		...
		http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				{
					AppContext.makeToast(ProductBatchList.this, "error_connectDataBase");
					return;
				}
				// 数据处理
				...
				case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
					...
					if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
					{
						...
						if (newdata > 0)
						{
							NewDataToast.makeText(ProductBatchList.this, getString(R.string.new_data_toast_message, newdata), appContext.isAppSound(), R.raw.newdatatoast).show();
						} else
						{
							// NewDataToast.makeText(ProductBatchList.this,
				...
				} else if (size == -1)
				{
					...
					AppContext.makeToast(ProductBatchList.this, "load_error");
				}
			...
			{
				AppContext.makeToast(ProductBatchList.this, "error_connectServer");
			}
		});
	}
	...
	{
		listAdapter = new ListViewProductBatchAdapter(ProductBatchList.this, listData);
		list_footer = ProductBatchList.this.getLayoutInflater().inflate(R.layout.listview_footer, null);
		...
		frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				if (planttab == null)
					return;
				Intent intent = new Intent(ProductBatchList.this, ProductBatchDetailList_.class);
				intent.putExtra("bean", planttab); // 因为list中添加了头部,因此要去掉一个
				ProductBatchList.this.startActivity(intent);
			}
		});
	...
	public void showPop_title()
	{
		...
		listview.setAdapter(new TitleAdapter(ProductBatchList.this, data));
	...
	public void onClick(View v)
	{
		...
		case R.id.btn_standardprocommand:
			intent = new Intent(ProductBatchList.this, AddStandardCommand_.class);
			startActivity(intent);
			break;
		case R.id.btn_nonstandardprocommand:
			intent = new Intent(ProductBatchList.this, AddNotStandardCommand_.class);
			startActivity(intent);
			break;
		case R.id.btn_nonprocommand:
			intent = new Intent(ProductBatchList.this, AddNotProductCommand_.class);
			startActivity(intent);
			break;
