...
import com.farm.R;
import com.farm.adapter.ListViewCostAdapter;
...
@SuppressLint("NewApi")
@EActivity(R.layout.ncz_costfragment)
public class NCZ_CostFragment extends Activity implements OnClickListener
{
    SelectorFragment selectorUi;
    Fragment mContent = new Fragment();
    private ListViewCostAdapter listAdapter;
    ...
    {
        // showPop_addcommand();
        Intent intent = new Intent(NCZ_CostFragment.this, AddSpecialCost_.class);
...
//		showPop_title();
    ...
    @CheckedChange(R.id.button_one)
    void button_one(CompoundButton hello, boolean isChecked)
    {
        if (isChecked)
        {
            Toast.makeText(NCZ_CostFragment.this, "button_one", Toast.LENGTH_SHORT).show();
        }
    }

    @CheckedChange(R.id.button_two)
    void button_two(CompoundButton hello, boolean isChecked)
    {
        if (isChecked)
        {
            Toast.makeText(NCZ_CostFragment.this, "button_two", Toast.LENGTH_SHORT).show();
        }
    }

    @CheckedChange(R.id.button_three)
    void button_three(CompoundButton hello, boolean isChecked)
    {
        if (isChecked)
        {
            Toast.makeText(NCZ_CostFragment.this, "button_three", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    void afterOncreate()
    {
        ...
        dictionary = DictionaryHelper.getDictionaryFromAssess(NCZ_CostFragment.this, "FY");
  ...
  /*  BroadcastReceiver receiver_update = new BroadcastReceiver()// 从扩展页面返回信息
    ...
    public void switchContent(Fragment from, Fragment to)
    {
        if (mContent != to)
        {
            ...
            if (!to.isAdded())
            { // 先判断是否被add过
                transaction.hide(from).add(R.id.top_container_cost, to).commit(); // 隐藏当前的fragment，add下一个到Activity中
            } else
            {
    ...
    {
        commembertab commembertab = AppContext.getUserInfo(NCZ_CostFragment.this);
        ...
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo)
            {
                ...
                {
                    AppContext.makeToast(NCZ_CostFragment.this, "error_connectDataBase");
                    return;
                }
                // 数据处理
                ...
                switch (actiontype)
                {
                    ...
                    case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
                        ...
                        if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
                        {
                            ...
                            if (newdata > 0)
                            {
                                NewDataToast.makeText(NCZ_CostFragment.this, getString(R.string.new_data_toast_message, newdata), appContext.isAppSound(), R.raw.newdatatoast).show();
                            } else
                            {
                                // NewDataToast.makeText(NCZ_CostFragment.this,
                ...
                } else if (size == -1)
                {
                    ...
                    AppContext.makeToast(NCZ_CostFragment.this, "load_error");
                }
            ...
            {
                AppContext.makeToast(NCZ_CostFragment.this, "error_connectServer");
            }
        });
    }
    ...
    {
        listAdapter = new ListViewCostAdapter(NCZ_CostFragment.this, listData);
        list_footer = NCZ_CostFragment.this.getLayoutInflater().inflate(R.layout.listview_footer, null);
        ...
        frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
        {
            public void onItemClick(AdapterView<?> parent, View view, int position, long id)
            {
                ...
                {
                    Intent intent = new Intent(NCZ_CostFragment.this, SpecialCostDetail_.class);
                    intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
                    startActivity(intent);
                } else if (planttab.getplantType().equals("1"))
                {
                    Intent intent = new Intent(NCZ_CostFragment.this, SmallCostDetail_.class);
                    intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
                    startActivity(intent);
                } else if (planttab.getplantType().equals("11"))
                {
                    Intent intent = new Intent(NCZ_CostFragment.this, LargeCostDetail_.class);
                    intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
                    startActivity(intent);
                }
            }
    ...
    public void showPop_title()
    {
        ...
        listview.setAdapter(new TitleAdapter(NCZ_CostFragment.this, data));
    ...
    public void onClick(View v)
    {
        ...
        switch (v.getId())
        {
            case R.id.btn_standardprocommand:
                intent = new Intent(NCZ_CostFragment.this, AddStandardCommand_.class);
                startActivity(intent);
                break;
            case R.id.btn_nonstandardprocommand:
                intent = new Intent(NCZ_CostFragment.this, AddNotStandardCommand_.class);
                startActivity(intent);
                break;
            case R.id.btn_nonprocommand:
                intent = new Intent(NCZ_CostFragment.this, AddNotProductCommand_.class);
                startActivity(intent);
                break;
