...
@SuppressLint("NewApi")
@EFragment
public class NCZ_CostFragment extends Fragment implements OnClickListener
{
	...
	void button_one(CompoundButton hello, boolean isChecked)
	{
		if (isChecked)
		{
			Toast.makeText(getActivity(), "button_one", Toast.LENGTH_SHORT).show();
		}
	}
	...
	void button_two(CompoundButton hello, boolean isChecked)
	{
		if (isChecked)
		{
			Toast.makeText(getActivity(), "button_two", Toast.LENGTH_SHORT).show();
		}
	}
	...
	void button_three(CompoundButton hello, boolean isChecked)
	{
		if (isChecked)
		{
			Toast.makeText(getActivity(), "button_three", Toast.LENGTH_SHORT).show();
		}
	}
	...
	void afterOncreate()
	{
		...
		dictionary = DictionaryHelper.getDictionaryFromAssess(getActivity(), "FY");
	...
	{
		View rootView = inflater.inflate(R.layout.ncz_costfragment, container, false);
	...
	{
		listAdapter = new ListViewCostAdapter(getActivity(), listData);
		...
		frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				{
					Intent intent = new Intent(getActivity(), SpecialCostDetail_.class);
					intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
					startActivity(intent);
				} else if (planttab.getplantType().equals("1"))
				{
					Intent intent = new Intent(getActivity(), SmallCostDetail_.class);
					intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
					startActivity(intent);
				} else if (planttab.getplantType().equals("11"))
				{
					Intent intent = new Intent(getActivity(), LargeCostDetail_.class);
					intent.putExtra("plant", planttab);// 因为list中添加了头部,因此要去掉一个
					startActivity(intent);
				}
			}
	...
	public void showPop_title()
	{
		...
		listview.setAdapter(new TitleAdapter(getActivity(), data));
