...
import com.farm.R;
import com.farm.adapter.ListViewEveryDayAssessAdapter;
...
@EActivity(R.layout.everydayassesslist)
public class EveryDayAssessList extends Activity implements OnClickListener
{
	...
	private ListViewEveryDayAssessAdapter listAdapter;
	...
	void btn_add()
	{
		Intent intent = new Intent(this, AddAssess_.class);
	...
	void afterOncreate()
	{
		dictionary = DictionaryHelper.getDictionaryFromAssess(EveryDayAssessList.this, "KH");
...
/*	BroadcastReceiver receiver_update = new BroadcastReceiver()// 从扩展页面返回信息
	...
	private void getListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		...
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.testurl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				{
					AppContext.makeToast(EveryDayAssessList.this, "error_connectDataBase");
					return;
				}
				...
				case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
					...
					if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
					{
						...
						if (newdata > 0)
						{
							NewDataToast.makeText(EveryDayAssessList.this, getString(R.string.new_data_toast_message, newdata), appContext.isAppSound(), R.raw.newdatatoast).show();
						} else
						{
							// NewDataToast.makeText(EveryDayAssessList.this,
				...
				} else if (size == -1)
				{
					...
					AppContext.makeToast(EveryDayAssessList.this, "load_error");
				}
			...
			{
				AppContext.makeToast(EveryDayAssessList.this, "error_connectServer");
			}
		});
	}
	...
	{
		listAdapter = new ListViewEveryDayAssessAdapter(this, listData);
