...
@EActivity(R.layout.addcommand)
public class AddStandardCommand extends Activity implements OnClickListener
{
	...
	void tv_selectcmd()
	{
		Dictionary_wheel dictionary_wheel = DictionaryHelper.getDictionary_Command(dic_comm);
		TwoWheel.showTwoWheel(this, tv_selectcmd, dictionary_wheel.getFirstItemID(), dictionary_wheel.getFirstItemName(), dictionary_wheel.getSecondItemID(), dictionary_wheel.getSecondItemName());
	}
	...
	void tv_area()
	{
		if (level.equals("0"))
		{
			...
			{
				Toast.makeText(AddStandardCommand.this, "获取数据异常！", Toast.LENGTH_SHORT).show();
			}
		...
		{
			Toast.makeText(AddStandardCommand.this, "请先选择执行级别！", Toast.LENGTH_SHORT).show();
		}
	}
	...
	void tv_workday()
	{
		JSONObject jsonObject = utils.parseJsonFile(AddStandardCommand.this, "dictionary.json");
		...
		OneWheel.showWheel(AddStandardCommand.this, firstItemid, number, tv_workday);
	}

	@Click
	void tv_timelimit()
	{
		MyDatepicker myDatepicker = new MyDatepicker(AddStandardCommand.this, tv_timelimit);
		...
		rg_level.setOnCheckedChangeListener(new OnCheckedChangeListener()
		{
			...
			public void onCheckedChanged(RadioGroup arg0, int arg1)
			{
				...
				if (radioButton.getText().equals("园区"))
				{
					...
					if (dic_area != null)
					{
						SqliteDb.deleteAllRecordtemp(AddStandardCommand.this, SelectRecords.class, dic_area.getBELONG());
					} else
					{
						Toast.makeText(AddStandardCommand.this, "获取数据异常！", Toast.LENGTH_SHORT).show();
					}
				...
				} else
				{
					...
					if (dic_park != null)
					{
						SqliteDb.deleteAllRecordtemp(AddStandardCommand.this, SelectRecords.class, dic_park.getBELONG());
					} else
					{
						Toast.makeText(AddStandardCommand.this, "获取数据异常！", Toast.LENGTH_SHORT).show();
					}
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		getActionBar().hide();
		commembertab = AppContext.getUserInfo(AddStandardCommand.this);
	}
	...
	private void commandTabAdd()
	{
		...
		{
			Toast.makeText(AddStandardCommand.this, "请选择区域！", Toast.LENGTH_SHORT).show();
			return;
		}
		String fi = "";
		String si = "";
		...
		if ((Bundle) tv_selectcmd.getTag() != null)
		{
			...
			fn = ((Bundle) tv_selectcmd.getTag()).getString("FN");
			sn = ((Bundle) tv_selectcmd.getTag()).getString("SN");
		} else
		{
		...
		{
			Toast.makeText(AddStandardCommand.this, "请选择重要性！", Toast.LENGTH_SHORT).show();
			return;
		}
		if (tv_workday.getText().toString().equals(""))
		{
			Toast.makeText(AddStandardCommand.this, "请选择作业天数！", Toast.LENGTH_SHORT).show();
			return;
		}
		if (tv_timelimit.getText().toString().equals(""))
		{
			Toast.makeText(AddStandardCommand.this, "请选择期限！", Toast.LENGTH_SHORT).show();
			return;
		}
		...
		params.addQueryStringParameter("stdJobType", fi);
		params.addQueryStringParameter("stdJobTypeName", fn);
		params.addQueryStringParameter("stdJobId", si);
		params.addQueryStringParameter("stdJobName", sn);
		...
		http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				if (result.getResultCode() == 1)// -1出错；0结果集数量为0；结果列表
				{
					...
					if (listData == null)
					{
						AppContext.makeToast(AddStandardCommand.this, "error_connectDataBase");
					} else
					{
						HaveReadRecord haveReadRecord = SqliteDb.getHaveReadRecord(AddStandardCommand.this, AppContext.TAG_NCZ_CMD);
						if (haveReadRecord != null)
						{
							SqliteDb.updateHaveReadRecord(AddStandardCommand.this, AppContext.TAG_NCZ_CMD, String.valueOf((Integer.valueOf(haveReadRecord.getNum()) + 1)));
						} else
						{
							SqliteDb.saveHaveReadRecord(AddStandardCommand.this, AppContext.TAG_NCZ_CMD, "1");
						}
						Toast.makeText(AddStandardCommand.this, "保存成功！", Toast.LENGTH_SHORT).show();
						finish();
					}
			...
			{
				AppContext.makeToast(AddStandardCommand.this, "error_connectServer");
			}
		});
	}
	...
	{
		commembertab commembertab = AppContext.getUserInfo(AddStandardCommand.this);
		RequestParams params = new RequestParams();
		params.addQueryStringParameter("uid", commembertab.getuId());
