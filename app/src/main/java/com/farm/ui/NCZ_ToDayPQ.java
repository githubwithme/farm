...
import com.farm.R;
import com.farm.adapter.NCZ_ToDayPQdapter;
 ...
 * @description :主界面
 */
@EActivity(R.layout.cz_pg_work)
public class NCZ_ToDayPQ extends Activity
{
    boolean ishidding;
    TimeThread timethread;
    private NCZ_ToDayPQdapter listAdapter;
    private int listSumData;
    private List<areatab> listData = new ArrayList<areatab>();
...
//        }
    ...
    private void getListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
    {
        ...
        params.addQueryStringParameter("action", "areaListGetByParkID");
        ...
        http.send(HttpRequest.HttpMethod.POST, AppConfig.testurl, params, new RequestCallBack<String>()
        {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo)
            {
                ...
                {
                    AppContext.makeToast(NCZ_ToDayPQ.this, "error_connectDataBase");
                ...
                switch (actiontype)
                {
                    ...
                    case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
                        ...
                        switch (objtype)
                        {
                            case UIHelper.LISTVIEW_DATATYPE_NEWS:
                                ...
                                if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
                                {
                                    ...
                                    {
                                        for (areatab areatab1 : listNewData)
                                        {
                                            boolean b = false;
                                            for (areatab areatab2 : listData)
                                            {
                                                if (areatab1.getid().equals(areatab2.getid()))
                                                {
                                                    b = true;
                                                    break;
                                                }
                        ...
                        if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
                        {
                            ...
                            if (newdata > 0)
                            {
                                NewDataToast.makeText(NCZ_ToDayPQ.this, getString(R.string.new_data_toast_message, newdata), appContext.isAppSound(), R.raw.newdatatoast).show();
                            } else
                            {
                            }
                        }
                ...
                } else if (size == -1)
                {
                    ...
                    AppContext.makeToast(NCZ_ToDayPQ.this, "load_error");
                }
            ...
            public void onFailure(HttpException error, String msg)
            {
                ...
                AppContext.makeToast(NCZ_ToDayPQ.this, "error_connectServer");
                if (!ishidding  && timethread!=null)
                {
                    timethread.setSleep(false);
                }
    ...
    {
        listAdapter = new NCZ_ToDayPQdapter(NCZ_ToDayPQ.this, listData);
        list_footer = NCZ_ToDayPQ.this.getLayoutInflater().inflate(R.layout.listview_footer, null);
        ...
        frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
        {
            public void onItemClick(AdapterView<?> parent, View view, int position, long id)
            {
                ...
                // if (animal == null)
                // return;
                areatab areatab = listData.get(position - 1);
                if (areatab == null) return;
                // Intent intent = new Intent(CZ_PG_Work.this,
                // CZ_Pg_WorkList_.class);
                // intent.putExtra("bean", areatab);// 因为list中添加了头部,因此要去掉一个
