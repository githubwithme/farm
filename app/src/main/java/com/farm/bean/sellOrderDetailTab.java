 ...
 * @version 1.0
 */
@Table(name = "sellOrderDetailTab")
public class sellOrderDetailTab implements Parcelable // 与数据库不一致
{
    public String id;
    ...
    public static final Creator<sellOrderDetailTab> CREATOR = new Creator()
    {
        @Override
        public sellOrderDetailTab createFromParcel(Parcel source)
        {
            // 必须按成员变量声明的顺序读取数据，不然会出现获取数据出错
            sellOrderDetailTab p = new sellOrderDetailTab();
        ...
        @Override
        public sellOrderDetailTab[] newArray(int size)
        {
            return new sellOrderDetailTab[size];
        }
    };
