...
@Table(name = "commembertab") public class commembertab_singleton implements Parcelable
{
    ...
    private static commembertab_singleton singleton = new commembertab_singleton();

    private commembertab_singleton()
    {
    }
    ...
    public static final Creator<commembertab_singleton> CREATOR = new Creator()
    {
        @Override public commembertab_singleton createFromParcel(Parcel source)
        {
            // 必须按成员变量声明的顺序读取数据，不然会出现获取数据出错
            commembertab_singleton p = new commembertab_singleton();
        ...
        @Override public commembertab_singleton[] newArray(int size)
        {
            return new commembertab_singleton[size];
        }
    };
